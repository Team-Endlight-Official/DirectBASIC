...
type TTexture2DHandle = type cardinal;
...
procedure BeginTexture2D(texture: TTexture2DHandle; slot: cardinal);
procedure EndTexture2D(slot: cardinal);
...
begin
    ...
    if slot >= GetMaxTextureSlots() then
    begin
        writeln('Texture2D slot is invalid! (Max: ' + IntToStr(GetMaxTextureSlots()) + ')');
        exit;
    end;
